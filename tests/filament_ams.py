...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='orca-filaments-') as directory:
        ...
        try:
            ...
            with sqlite3.connect(tmp/'plates/orca.sqlite3') as c:assert c.execute('PRAGMA user_version').fetchone()==(2,)
            (output/'verified.json').write_text(json.dumps(dict(schema=2,printers=2,filaments=4,initial_layer=250,normal=240,matte_black_white=True,missed_swap_invalidates=True,restart_unconfirmed=True,isolated_commands='status-only'),indent=2))
        finally:
