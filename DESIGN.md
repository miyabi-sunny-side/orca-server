...
メニューの「フィラメント」から銘柄・色の台帳を開く。一覧は色見本、名前、メーカー、材料種別の順に並べ、
「追加」だけを主ボタンとする。PETGとPETG-GFの文字を省略しない。色見本にはRGBAの文字も添える。
材料の編集画面から機種・ノズルごとの設定を追加する。既定profileと初層・通常温度の差分を分け、
空欄はprofileを使う意味をラベルで示す。ベッド温度も初層・通常層を個別に指定でき、0℃は加熱なしとする。
ベッドの明示値はプレート種別にかかわらず適用されることを設定欄で示す。対応profileがない構成を、別の径の設定で補わない。
保存失敗は入力を維持し、削除確認には材料名を含める。参照中の削除は理由と復帰操作を表示する。
