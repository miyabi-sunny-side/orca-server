...
詳細は配置図、モデル名と設定、印刷キューへの入口、3MFのダウンロードを示す。
...
詳細の主操作は見出し直下の「印刷キューへ」。生成済み3MFの取得と元モデルの再取得は補助操作とする。
再取得すると現在の設定で配置・スライスを作り直すことを操作付近で伝える。

### 印刷キュー

メニューの「印刷キュー」は`/queue`を開く。プレート詳細の主操作は`/queue?plate=<id>`を開き、
名前・AMSのnative select・「キューに追加」を表示する。AMSは1始まりのユニットとスロット、
取得できた材料を表示する。空と未確認を区別し、未接続でも選択して追加できる。
選択は初期値を設けず、利用者が指定する。追加によりモデル・設定・生成データが固定されることと、
サーバー再起動でキューが消え保存済みプレートは残ることを、その操作の近くへ示す。
成功後は通常のキューへ戻り、追加フォームを残さない。
...
待機行には名前・AMSと、順序変更・削除の操作を置く。ページを通常スクロールする。
印刷中や開始確認中は次を開始できず、完了だけでは自動で進めない。

「造形物を取り外し、空のビルドプレートを戻しました」のcheckboxで確認してから、
「次を印刷」を押す。取得した世代・対象・要求IDを送信し、サーバーの既存判定を必ず通す。
...
新規プレートでは選択した機器に合う工程・材料を表示する。配置図の範囲は保存3MFのprintable_areaに従う。
両テーマ、320/375/900px、長い名前、空台帳、保存失敗からの復帰、キーボード、文字200%で実測する。

### Materials and AMS

メニューの「材料」から銘柄・色の台帳を開く。一覧は色見本、名前、メーカー、材料種別の順に並べ、
