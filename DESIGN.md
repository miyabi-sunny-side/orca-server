...
開始時に最新のSCADモデルと現在の配備済みプロファイルを取得し、配置・スライスする。
...
直接アップロードしたモデルは元STLの取得リンクを持つ。SCAD参照は準備開始時に取得することを短く伝える。
...
4項目は未設定でも保存でき、未設定または無効な組合せを別の値へ自動変更しない。未設定は「未設定」と表示する。
...
通常画面は、プリンター状態・現在のジョブ・次の名前・取り外し確認・開始操作・待機一覧の順とする。
現在はsurface-raisedの面、次は本文の一行で示す。全画面の告知や強い装飾を追加しない。
375×812の代表データでは、現在名・状態・次名・確認・次操作と待機先頭1件をスクロールなしで読める。
...
「造形物を取り外し、空のビルドプレートを戻しました」のcheckboxで確認してから、
「次を印刷」を押す。取得したサーバー識別子・世代・対象・要求ID・取り外し対象IDを送信し、サーバーの既存判定を必ず通す。
...
サーバー識別子・世代・開始可能状態の変化や取得失敗で確認を解除する。
...
再実行と次の開始を同時に主操作にしない。次がなければ取り外し完了だけで現在を閉じられる。
