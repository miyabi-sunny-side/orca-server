...
ホームは保存済みプレートを検索して開く入口とする。プレートは名前・STLの参照・個数を保存する。
新規作成はモデル選択→名前と個数の確認→保存→詳細。詳細から構成を編集し、印刷キューへ進む。
...
メニューの「印刷キュー」は`/queue`を開く。プレート詳細からは`/queue?plate=<id>`へ進む。
機器・使用予定の材料・AMSスロット・要求する機種/ノズルをnative selectで選ぶ。
工程（品質）とベッド条件は機器の既定値から始め、要求機種を変えたら対応する工程を選び直す。
品質・ベッド条件は保存前も常時表示する。材料は銘柄・材料種別・色で区別する。
AMSの現在の割当と予定材料は別々に表示する。スロットはAMS IDと1始まりのスロット番号で示す。
材料の対応設定が無い場合は追加を止め、材料設定へのリンクを示す。未同期のAMSを確認済みと表示しない。
追加成功後は通常キューへ戻る。追加や待機設定の保存は印刷開始を伴わない。
...
待機行には名前・予定材料・AMS・要求ノズルと、設定変更・順序変更・取消の操作を置く。
不一致は「保留」と理由で示し、待機したまま編集できる。設定編集中に他画面で更新されても上書きしない。ページを通常スクロールする。
...
キューの追加・設定変更では、要求機種に合う工程と材料設定を表示する。
...
100件と長い名前、取得失敗の再試行、選択・構成保存・再検索・再読み込み・待機設定の編集・印刷準備・取り外しを実測する。
