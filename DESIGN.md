...
description: OrcaServerの接続画面と明暗テーマのデザイン契約。
colors:
  primary: "#245d84"
  accent: "#245d84"
...
現在の画面は接続確認とテーマ選択を提供する。デモ項目や未実装の操作は表示しない。
右上のメニューからテーマ設定を開き、変更結果をその場で確認して画面へ戻る。
接続失敗時は同じ場所に再試行を置き、成功したらエラー表示を置き換える。
...
本文先頭に接続状態を14pxで表示する。画面の状態はloading/error/successとする。
読込中はスピナーと「接続を確認しています…」を控えめに示す。
成功は「OrcaServerに接続しました」、失敗は「接続できませんでした」と表示する。
失敗はdanger色とrole=alertで伝え、直下に「再試行」を置く。
状態の取得は画面を開いた時と再試行時に行い、未実装の機能を操作可能に見せない。
...
接続状態は320px幅でも上端120px以内に表示し、横にはみ出させない。
`npm --prefix client run test:e2e` で320px・375px・900pxの明暗を確認する。
接続待ち・失敗から再試行、テーマの保存とOS追従、キーボードと閉じる操作を実測する。
