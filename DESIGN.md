...
プリンター一覧の「AMS」から、その機器の装填情報を開く。上部のnative selectで機器を切り替える。
機器内のAMS・スロットごとに、台帳の材料と対応方法、本体の報告、設定温度を別に示す。
自動対応・手動対応・未指定を文字で区別し、タグなしの同値材料を自動判別できる表現にしない。
残量不明を0%と表示しない。報告された温度範囲を、初層・通常温度の設定欄へ転記しない。
起動直後・切断・同期前は保存情報と最終観測時刻を表示し、現在の装填確認とは区別する。
手動対応は台帳から選んで保存し、割当解除と再指定ができる。古い観測での保存は再確認へ戻す。
ノズルの適合状態は登録構成からの判定であり、装着センサーの確認とは扱わない。

既存の720px本文、8pxカード、native入力、両テーマtokenを使う。データの色を主操作色へ転用しない。
320/375/900pxと文字200%で、Matte黒白・手動PETG-GF・残量不明・未接続を並べて実測する。
