...
「次を印刷」で準備を開始すると、モデルをOrcaSlicerで配置し、編集用`project.3mf`と印刷用`print.gcode.3mf`を生成します。
...
理由と本体を確認して[キューから復旧](queue.md#失敗・再起動からの復旧)してください。
...
展開した基本設定に、DBに保存したノズル温度の明示的な上書きだけを適用します。
