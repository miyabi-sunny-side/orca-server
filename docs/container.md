...
手元のSTL・3MFは「プレート」→「ファイル取込」で保存できます。追加の接続設定は不要です。
scad-liveのモデルを選ぶには、コンテナから到達できるURLを`SCAD_LIVE_URL`へ指定します。
...
インフィル・充填率・壁の枚数も旧プレートではNULLのままとし、従来の工程設定を継承します。
新規プレート用の初期値はadaptivecubic・15%・2周です。進行中の実行入力や生成物は書き換えません。
