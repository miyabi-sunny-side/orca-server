...
test('late defaults preserve deliberate selections and clearing, without repeated refresh',async({page})=>{
  ...
  const material=page.getByRole('combobox',{name:'フィラメント',exact:true});await material.selectOption('blue');await material.selectOption({label:'未設定'});
  ...
  await expect(material).toHaveValue('');await expect(page.getByRole('combobox',{name:'ビルドプレート',exact:true})).toHaveValue('High Temp Plate');
  await material.selectOption('blue');await page.getByRole('button',{name:'モデル選択へ'}).click();await page.getByRole('button',{name:'構成を確認（1）'}).click();
  await expect(material).toHaveValue('blue');expect(reads).toBe(1);
});
