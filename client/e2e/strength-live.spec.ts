...
test('details stay optional, persist strength and calculate shells in both themes',async({page,request})=>{
  ...
  for(const [width,colorScheme] of [[320,'dark'],[900,'light']] as const){
    ...
    expect((await (await request.get('/api/plates/'+id)).json()).conditions).toMatchObject({sparse_infill_pattern:'gyroid',sparse_infill_density:22.5,wall_loops:3});
...
test('late defaults and failed saves preserve manual input; legacy values come from the process',async({page,request})=>{
  ...
  await page.getByLabel('壁の枚数（周）').fill('4');await page.getByLabel('充填率（%）').fill('0');
  ...
  await expect(page.getByLabel('プレート名',{exact:true})).toHaveValue('入力を保持');
  await expect(page.getByLabel('壁の枚数（周）')).toHaveValue('4');await expect(page.getByLabel('充填率（%）')).toHaveValue('0');
